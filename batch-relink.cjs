...
// Map campuses by normalized university name
...
for (let file of files) {
    ...
    progs.forEach(p => {
        ...
        } else if (uniCampuses.length > 0) {
            // If completely unmatched, the user wants us to remove the fallback!
            // Actually, we should create it. But wait, we already created ALL missing MYOs in smart-geocode.cjs!
            // So if it's unmatched, it's either a main campus Faculty or it was already matched to Main.
            // Let's just assign it to Main Campus!
            let mainCampus = uniCampuses.find(c => c.isMain) || uniCampuses[0];
            if (p.campus_id !== mainCampus.id || p.campus_name !== mainCampus.name) {
                p.campus_id = mainCampus.id;
                p.campus_name = mainCampus.name;
                isModified = true;
                updatedProgramsCount++;
            }
        }
...
console.log('Total programs checked:', totalPrograms);
console.log('Programs explicitly linked to campuses (re-linked):', updatedProgramsCount);
