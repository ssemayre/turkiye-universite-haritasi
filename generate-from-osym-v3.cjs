/**
 * generate-from-osym-v3.cjs
 * =======================
 * Tablo-4 (Lisans) Fakültelerini TEK PİN ("Merkez Kampüs") altında birleştirir.
 * Temizlenmiş Üniversite isimleriyle kademeli (3 Aşamalı) arama yapar.
 */
...
function cleanUniName(name) {
    ...
    // Yaygın ÖSYM İsim Kirlilikleri (Baştaki il isimlerini sil)
    ...
    for (const prefix of prefixesToRemove) {
        if (clean.toUpperCase().startsWith(prefix)) {
            const replacement = prefix.split(' ').slice(1).join(' '); // İlk kelimeyi (İl adını) at
...
// Tablo-4 (Lisans) Fakülteleri için Merkez Kampüs Kuralı
function isMainCampusFaculty(facultyName) {
    const upper = facultyName.toUpperCase();
    
    // Yüksekokul veya MYO ise BAĞIMSIZDIR
    if (upper.includes('YÜKSEKOKUL') || upper.includes('MYO')) return false;
    
    // Parantez içinde özel bir ilçe/yer adı varsa BAĞIMSIZDIR
    ...
    if (bracketMatch) {
        ...
        if (!inside.includes('İNGİLİZCE') && !inside.includes('UÖ') && !inside.includes('AÇIKÖĞRETİM') && !inside.includes('ÜCRETLİ') && !inside.includes('BURSLU')) {
            return false; // İlçe adıdır (Örn: Kahta, Biga vb.)
        }
    }

    // Diğer tüm Lisans (Tablo-4) Fakülteleri Merkez Kampüse aittir!
    return true; 
}
...
async function main() {
    ...
    for (let i = 0; i < allCampuses.length; i++) {
        ...
        // 1. Deneme: Sadece MYO/Fakülte Adı + İl (Örn: "Bozdoğan Meslek Yüksekokulu, Aydın")
        const q1 = `${targetName}, ${city}`;
        
        // 2. Deneme: Temiz Üniversite Adı + MYO (Örn: "Adnan Menderes Üniversitesi Bozdoğan Meslek Yüksekokulu")
        let q2 = `${cleanUni} ${targetName}`;
        if (campus.isMain) q2 = `${cleanUni} Merkez Kampüsü ${city}`;
        
        // 3. Deneme: Sponsor İsimsiz (Örn: "Karacasu Meslek Yüksekokulu, Aydın")
        ...
        try { await sleep(SLEEP_MS); result = await geocodeLocationIQ(q1); queryUsed = 'Q1'; } catch(e) {}
        
        if (!result) {
            try { await sleep(SLEEP_MS); result = await geocodeLocationIQ(q2); queryUsed = 'Q2'; } catch(e) {}
        }
        
        if (!result && noSponsorName !== targetName) {
            try { await sleep(SLEEP_MS); result = await geocodeLocationIQ(q3); queryUsed = 'Q3'; } catch(e) {}
        }
    ...
    console.log('\n=== ÖSYM TABANLI KADEMELİ ÜRETİM (V3) SONUÇLARI ===');
